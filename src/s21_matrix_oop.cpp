...
S21Matrix::S21Matrix(int n) : rows_(n), cols_(n) {
  if (n <= 0) throw std::invalid_argument("Invalid size for a square matrix");
  matrix_ = new double[rows_ * cols_]{};
  SetAllToZero();
}

S21Matrix::S21Matrix(int r, int c) : rows_(r), cols_(c) {
  if (r <= 0 || c <= 0) throw std::invalid_argument("Invalid size");
  matrix_ = new double[rows_ * cols_]{};
  SetAllToZero();
}
...
S21Matrix::S21Matrix(const S21Matrix& other)
  ...
  if (rows_ <= 0 || cols_ <= 0) throw std::invalid_argument("Invalid size");
  matrix_ = new double[rows_ * cols_]{};
...
void S21Matrix::setRows(int number) {
  if (number < 1) throw std::invalid_argument("Invalid number of rows");
...
void S21Matrix::setCols(int number) {
  if (number < 1) throw std::invalid_argument("Invalid number of columns");
  ...
  if ((rows_ != other.getRows()) || (cols_ != other.getCols()))
    ...
    for (int i = 0; i < rows_ && i < other.getRows(); i++)
      for (int j = 0; j < cols_ && j < other.getCols(); j++) {
        matrix_[i * cols_ + j] =
            matrix_[i * cols_ + j] + other.matrix_[i * other.getCols() + j];
      }
  }
}
    ...
    for (int i = 0; i < rows_; i++) {
      for (int j = 0; j < other.getCols(); j++) {
        for (int k = 0; k < other.getRows(); k++) {
          result.matrix_[i * other.getCols() + j] +=
              matrix_[i * cols_ + k] * other.getCell(k * other.getCols() + j);
        }
      }
    }
    *this = result;
...
S21Matrix S21Matrix::InverseMatrix() {
  int det = Determinant();
  if (fabs(det) < S21_EPS)
    throw std::logic_error("Can't inverse: determinant equals 0");
  ...
  if ((rows_ != other.getRows()) || (cols_ != other.getCols()))
    ...
    for (int i = 0; i < rows_ && ret; i++)
      for (int j = 0; j < cols_ && ret; j++) {
        if (fabs((*this)(i, j) - other.getCell(i * cols_ + j)) > S21_EPS)
          ret = false;
      }
  return ret;
}
...
double S21Matrix::operator()(int r, int c) {
  if (r < 0 || c < 0 || r >= rows_ || c >= cols_)
    throw std::out_of_range("Index outside of matrix");
  else
